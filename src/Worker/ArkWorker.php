...
class ArkWorker extends BaseWorker
{
    use QueueJobTrait;
    ...
    public function flood($payload, $job)
    {
        ...
        $payload['first'] = false;
        $sequence = rand(0, 9) > 3 ? null : 'ark_' . $job->id . '_' . time();
        for ($i = 0; $i < $number_forks; $i++) {
            $this->_queueJob('FloodTest', 'DelayedJobs\Worker\ArkWorker', 'flood', $payload, 20, $sequence);
        }
