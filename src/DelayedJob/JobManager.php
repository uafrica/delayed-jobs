...
class JobManager implements EventDispatcherInterface, ManagerInterface
{
    ...
    public function loadJob(Job $job)
    {
        ...
        if ($job->getBrokerMessageBody() && !$job->getPayload('brokerMessageBody')) {
            $job->setPayloadKey('brokerMessageBody', $job->getBrokerMessageBody());//If there is a message body, ensure that it's not lost on a retry!
        }
    }
    ...
    protected function _handleResult(ResultInterface $result, $duration)
    {
        ...
        if ($result->getRetry() && $job->getRetries() < $job->getMaxRetries()) {
            ...
            $job->setRunAt($this->_calculateRetryTime($job->getRetries()));
